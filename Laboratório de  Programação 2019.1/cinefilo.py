...
class AVL(ArvoreBinariaBusca):
    ...
    def definirAltura(self,no):
        if no is not None:
            ...
            no.setAltura(1 + self.maior(d,e))
            return 1 + self.maior(d,e)
        return -1
...
for i in range(generos):
    ...
    while True:
        filmes = input().split()
        if filmes[0] == "I":
            arvore.inserirAVL(int(filmes[1]))
        elif filmes[0] == "N":
            nivel = arvore.buscarNivel(int(filmes[1]))
            resposta += str(nivel) + "\n"
        elif filmes[0] == "L":
            res = ""
            lista = arvore.printFilmes(int(filmes[1]),int(filmes[2]),arvore.getRaiz()) 
            resposta += lista +"\n"
        elif filmes[0] == "F":
            resposta += "\n"
            break
